...
impl RegenCommand {
    pub fn run(self) -> Result<RegenReport> {
        ...
        fs::write(&workflow_path, yaml).with_context(|| {
            format!("failed to write workflow file `{}`", workflow_path.display())
        })?;
...
fn analyze_project(project_root: &Path) -> Result<ProjectAnalysis> {
    if !project_root.is_dir() {
        bail!("project directory `{}` does not exist", project_root.display());
    }
    ...
    let (kind, name) = if cargo_toml.exists() {
        ...
        let package_name = parse_package_name(&manifest).unwrap_or_else(dir_name);
    ...
    let validation_commands = match kind {
    ...
    Ok(ProjectAnalysis { root: project_root.to_path_buf(), name, kind, notes, validation_commands })
}

// ─── YAML renderer ────────────────────────────────────────────────────────────

// format!() instead of serde_yaml so we preserve block scalars, YAML comments, and key order.
fn render_workflow_yaml(analysis: &ProjectAnalysis, custom_preferences: Option<&str>) -> String {
    ...
    let build_commands: String = analysis
        .validation_commands
        .iter()
        .map(|cmd| format!("      - \"{}\"\n", sanitize_yaml_scalar(cmd)))
        .collect();

    let has_prefs = custom_preferences.map(|v| !v.trim().is_empty()).unwrap_or(false);
    ...
    let plan_snippets      = render_step_snippets(&["project_brief"], has_prefs);
    let review_snippets    = render_step_snippets(&["review_brief"],  has_prefs);
    let implement_snippets = render_step_snippets(&[],                has_prefs);
...
fn prompt_for_preferences() -> Result<Option<String>> {
    if !prompt_yes_no("Do you want to add custom preferences before writing workflow.yaml?", false)? {
        return Ok(None);
    }
...
fn prompt_yes_no(message: &str, default: bool) -> Result<bool> {
    // Uppercase letter shows which answer Enter selects.
...
fn prompt_line(prompt: &str) -> Result<String> {
    print!("{prompt}");
    // flush() required — stdout is line-buffered, so print! without \n won't appear before read_line blocks.
    io::stdout().flush().context("failed to flush stdout")?;

    let mut line = String::new();
    io::stdin().read_line(&mut line).context("failed to read user input")?;
...
// Prevents user-controlled strings from breaking the YAML structure.
...
fn detect_rust_commands(project_root: &Path) -> Result<Vec<String>> {
    ...
    if manifest.contains("clippy") || project_root.join("clippy.toml").exists() {
        // clippy before tests so lint failures surface before a potentially slow test run
        commands.push("cargo clippy --all-targets --all-features -- -D warnings".to_string());
    }

    commands.push(if manifest.contains("[workspace]") {
